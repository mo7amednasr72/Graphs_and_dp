...
void init(int n){
    FORI(0, n) { cost[i] = INT_MAX; adjacency_list[i].clear(); }
}
...
void dijkstra(int src){
    ...
    while(!pq.empty()){
        ...
        for( auto &child : adjacency_list[curr_node] ){
            int child_node = child.first, edge_cost = child.second;
...
void burn(int k){
    ...
    if(cost[e] == INT_MAX) cout << "unreachable\n";
    else cout << cost[e] << '\n';
